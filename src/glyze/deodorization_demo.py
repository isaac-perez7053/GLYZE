...
def vapor_pressure(numCarbons, Temperature):
    ...
    R = 8.314e-3 #kJ/mol-K
    vapor_pressure = scipy.exp(-delta_H_vap/R * (1/Temperature - 1/T_1mm) + scipy.log(T_1mm))
...
def evaporation_rate(x, molar_mass, numCarbons, Temperature):
    ...
    EVRT = N_A*gamma*x*Pvap*scipy.sqrt(2*scipy.pi*R*molar_mass*Temperature) #molecules/m2-s
    return EVRT
